...
```typescript
const add = Command.make("add", { task }, ({ task }) => Console.log(`Adding: ${task}`)).pipe(
  Command.withDescription("Add a new task"),
);

const list = Command.make("list", {}, () => Console.log("Listing tasks...")).pipe(
